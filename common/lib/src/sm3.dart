...
class SM3 {
  ...
  static String hashString(String data, {String? key}) {
    if (key != null) {
      return _hmac(data, key);
    }
    return bytesToHexString(_hashBytesToBytes(utf8.encode(data)));
  }
  ...
  static String hashBytes(List<int> data, {String? key}) {
    ...
    return bytesToHexString(_hashBytesToBytes(data));
  }

  static List<int> _hashBytesToBytes(List<int> array) {
    ...
    for (int i = 0; i < numberOfBlocks; i++) {
      ...
      for (int j = 16; j < 68; j++) {
        W[j] = (_P1((W[j - 16] ^ W[j - 9]) ^ leftShift(W[j - 3], 15)) ^
        leftShift(W[j - 13], 7)) ^
        W[j - 6];
      }
      ...
      for (int j = 0; j < 64; j++) {
        T = j >= 0 && j <= 15 ? _T1 : _T2;
        SS1 = leftShift(
            leftShift(A, 12) + E + leftShift(T, j), 7);
  ...
  static String _hmac(dynamic input, String key) {
    ...
    Uint8List keyBytes = Uint8List.fromList(hexStringToBytes(key));

    if (keyBytes.length > 64) {
      keyBytes = Uint8List.fromList(_hashBytesToBytes(keyBytes));
    } else {
      final padding = 64 - keyBytes.length;
      keyBytes = Uint8List.fromList(keyBytes + List.filled(padding, 0));
    }

    final iPadKey = _xor(keyBytes, iPad);
    final oPadKey = _xor(keyBytes, oPad);
    ...
    return hashBytes(
        Uint8List.fromList(oPadKey + _hashBytesToBytes(iPadKey + inputBytes)));
  }
  ...
  static List<int> _xor(List<int> x, List<int> y) {
